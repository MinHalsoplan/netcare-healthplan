...
public class AuthenticationFilterQA extends AbstractPreAuthenticatedProcessingFilter {

private static final Logger log = LoggerFactory.getLogger(AuthenticationFilterQA.class);
	...
	protected Object getPreAuthenticatedPrincipal(HttpServletRequest request) {
		...
		if (auth == null) {
			...
			if (guid != null) {
				log.debug("Guid parameter found. Guid is: {}. Validate it against MVK...", guid);
				
				try {
					//this.service.authenticate(AuthenticationRequestImpl.newAuthenticationRequest(guid));
				} catch (Exception e) {
					// OK
				}
				
				if (request.getParameter("hsaId") != null) {
					return AuthenticationResultImpl.newCareGiver(request.getParameter("hsaId"), request.getParameter("careUnitHsaId"), request.getParameter("careUnitName"));
				} else {
					return AuthenticationResultImpl.newPatient(request.getParameter("civicRegistrationNumber"));
				}
			}
		} else {
