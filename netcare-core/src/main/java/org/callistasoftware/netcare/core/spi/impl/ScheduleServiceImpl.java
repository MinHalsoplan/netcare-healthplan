...
public class ScheduleServiceImpl extends ServiceSupport implements ScheduleService {
	...
	@Override
	public ServiceResult<ScheduledActivity[]> load(final boolean includeReported, final boolean includeDue, Long start,
			Long end) {
		...
		for (final ScheduledActivityEntity sae : list) {
			...
			boolean reportCheck = sae.getReportedTime() != null
					&& (sae.getStatus().equals(ScheduledActivityStatus.OPEN) || sae.getStatus().equals(
							ScheduledActivityStatus.REJECTED));
	...
	@Override
	public ServiceResult<ScheduledActivity> loadLatestForDefinition(Long definitionId) {
		final List<ScheduledActivityEntity> all = this.repo.findLatestScheduledActivityForActivity(definitionId);
		if (all.isEmpty()) {
			throw new IllegalStateException("Could not find any scheduled activities for definition " + definitionId
					+ ". This method should never be invoked at this stage.");
		}

		final ScheduledActivityEntity act = all.get(0);
		act.isReadAllowed(getPatient());

		return ServiceResultImpl.createSuccessResult(ScheduledActivityImpl.newFromEntity(act),
				new GenericSuccessMessage());
	}
	...
	private ActivityItemValuesEntity findItemValue(final ScheduledActivityEntity scheduledActivity, final Long id) {
		...
		throw new IllegalArgumentException("Could not find item value " + id + " in scheduled activity "
				+ scheduledActivity.getId());
	}
	...
	public ServiceResult<ScheduledActivity> reportReady(final ScheduledActivity report) {
		...
		HealthPlanEntity healthPlan = entity.getActivityDefinitionEntity().getHealthPlan();
		if (!healthPlan.isActive()) {
			throw new IllegalStateException("Cannot report when health plan is inactive. Scheduled activity: "
					+ entity.getId());
		}

		if (entity.getActivityDefinitionEntity().isRemovedFlag()) {
			throw new IllegalStateException("Cannot report when activity definition is inactive. Scheduled activity: "
					+ entity.getId());
		}
		...
		for (final ActivityItemValues value : report.getActivityItemValues()) {
			...
			if (valueEntity instanceof MeasurementEntity) {
				...
				if (e.getPerceivedSense() == null) {
					...
					/*
					 * Just put an average sense value here if perceived sense
					 * is null and and the item is excluded
					 */
					final EstimationTypeEntity ed = (EstimationTypeEntity) ee.getActivityItemDefinitionEntity()
							.getActivityItemType();
				...
				if (yn.getAnswer() == null) {
					/*
					 * This case doesn't really matter at the moment. Since the
					 * yes/no item has been excluded we can safely set this to
					 * false in order to not pollute the database with invalid
					 * null values.
					 */
					yne.setAnswer(false);
				} else {
