...
DECLARE
       ...
       rec RECORD;
BEGIN
	FOR rec in select u.id, u.email, u.first_name, u.sur_name, g.hsa_id, e.hsa_id ehsa_id from nc_user u, nc_care_giver g, nc_care_unit e  where g.id = u.id and e.id = g.care_unit_id
	LOOP
